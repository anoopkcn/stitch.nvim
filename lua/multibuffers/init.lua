-- multibuffers.nvim — edit excerpts from many files in one view.
--
...
-- edits) and then renders a grouped, editable multibuffer view on top of it.
local M = {}
...
local function present(items, title)
  ...
  show(require('multibuffers.model').from_items(items, title))
end
...
function M.from_qflist()
  ...
  show(require('multibuffers.model').from_items(qf.items, qf.title))
end
